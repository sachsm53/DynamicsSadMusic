###UPDATED ON 10/03/2019 BY MATT SACHS

basepath = '/Volumes/MusicProject/NaturalisticNetwork'
if (!file.exists(basepath)){
  basepath = '/Volumes/MusicProject-1/NaturalisticNetwork'
}

survey <- read.csv(file.path(basepath,'results_data_tables/naturalistic_network_survey_results.csv'))
survey$subject <- as.factor(survey$subject)
G1Subj <- survey$subject[which(survey$GROUP_FANTASY == 0)]
G1Subj <- as.character(G1Subj)
G2Subj <- survey$subject[which(survey$GROUP_FANTASY == 1)]
G2Subj <- as.character(G2Subj)
inFile <- file.path(basepath,'group_compare/pairwise_data_36ss_aroma_stand.txt')
outFile <- file.path(basepath,'group_compare/LME_18grp_stand_fantasy_g1-g2')


##GROUP 1 HIGH IRI - FANTASY
G1Subj <- c('sub-15','sub-06','sub-08','sub-36','sub-12','sub-21','sub-40','sub-33','sub-16','sub-25','sub-24','sub-02','sub-20','sub-29','sub-34','sub-26','sub-23','sub-10')
G1Subj <- sort(G1Subj)

##GROUP 2 - LOW IRI - FANTASY
G2Subj <- c('sub-03','sub-08','sub-10','sub-11','sub-12','sub-13','sub-14','sub-17','sub-21','sub-22','sub-24','sub-29','sub-31','sub-33','sub-34','sub-35','sub-37','sub-40')
G2Subj <- sort(G2Subj)
...
file = '/Users/msachs/abin/AFNIio.R'
source(file)
...
tolL <- 1e-16 # bottom tolerance for avoiding division by 0 and for avioding analyzing data with most 0's
#allData <- array(apply(allData, 4, function(x) x*(abs(maskData)>tolL)),
#      dim=c(dimx,dimy,dimz,nFile))
allData <- r2z(allData)
...
mylme <- function(myDat, n1, n2, NN1, NN2, NN12) {
   if (!all(abs(myDat) < 10e-8)) {
      ...
      cc <- fm$coefficients
      # later on move the 7 C* outside #THIS WAS CHANGE (CONTRAST 7) ON 10/03/2019
